# program med komplisert p og enkel p, og finpussa plot
import numpy as np
...
"""
def p(t):
    p_arr = np.zeros(len(t))
    max_t = max(t)
    for i in range(len(t)):
        if t[i] < 0.4*max_t:
            p_arr[i] = p0*np.sin(omega*t[i])
        else:
            p_arr[i] = 0
    return p_arr
"""
...
plt.plot(x,eta[80000,:],'--',color = 'grey',label = '{:.3e}s'.format(t[80000]))
plt.plot(x,eta[maxt_in[0][0]],color = 'green',label = '{:.3e}s'.format(t[maxt_in[0][0]]))
...
f_eigen = np.sqrt(k/m)*1/(2*np.pi)

plt.figure()
plt.plot(f_eigen,eta[-1,:]) # plotter mot egenfrekvensen
plt.xlabel('Frekvens [Hz]')
plt.ylabel('Amplitude [cm]')
plt.axvline(x = f_eigen[max_in[0][0]],color = 'r', linewidth = 0.5,label ='f:{:.3e}'.format(f_eigen[max_in[0][0]]))
plt.legend()
plt.show()
...
plt.plot(x,xi[80000,:],'--', label = '{:.3e}s'.format(t[80000]))
