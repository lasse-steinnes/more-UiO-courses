# program med enkel p og enkel plot
...
plt.figure()
plt.plot(x, eta[-1,:])
plt.axvline(x = x[max_in],color = 'r', linewidth = 0.5,label ='x:{:.3e}'.format(x[max_in]))
...
plt.figure()
plt.plot(t, eta[:,max_in[0][0]])
plt.axvline(x = t[maxt_in[0][0]])
plt.xlabel('Tid [s]')
plt.ylabel('Amplitude [cm]')
plt.show()
...
# plotter trykksforskjellen langs membranen
plt.figure()
